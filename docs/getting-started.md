...
`bunx @brazilian-utils/brazilian-utils` and `deno run -A npm:@brazilian-utils/brazilian-utils` run the same command, and once the package is installed in a project it is `npx brazilian-utils` (or plain `brazilian-utils` in a `package.json` script).
...
| `--flag`, `--no-flag`        | A boolean option set to `true` or to `false`: `formatCurrency 10 --no-symbol`                                 |
...
Strings and numbers are printed as they are, anything else as JSON. The exit code is `0` on success, `1` when the answer is `false` or `null` or the utility throws (the error goes to stderr), and `2` when the command line itself is wrong, so a validator works as a shell condition:
