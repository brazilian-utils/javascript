...
Hides most of a Pix key with `*`, for the places where a key is shown to someone who should only recognize it, such as a list of registered keys (LGPD, art. 6º III, necessidade). The key is identified by `getPixKeyInfo`, under the rules documented there, and each kind is hidden by the utility that already knows how: a CPF by `formatCpf` with `obfuscate` (`***.456.789-**`, the form the Banco Central prints for a "CPF mascarado" in the [Pix user experience manual](https://www.bcb.gov.br/content/estabilidadefinanceira/pix/Regulamento_Pix/IV_RequisitosMinimosparaExperienciadoUsuario.pdf)), a CNPJ by `formatCnpj` with `obfuscate`, a phone by `formatPhone` with the `"international"` mask and `obfuscate`, and an e-mail by `obfuscateEmail` over the lowercased address. A random key (EVP) is returned whole, lowercased: the [DICT manual](https://www.bcb.gov.br/content/estabilidadefinanceira/pix/Regulamento_Pix/X_ManualOperacionaldoDICT.pdf) defines it as a sequence "que não possui qualquer significado, a não ser o de servir como uma chave Pix", so it carries no personal data to hide. Returns an empty string when the value is not a valid Pix key. The user experience manual forbids masking the key on the screen where a payer confirms the recipient, so this is not meant for that screen.
...
Format PIS number. `options.pad` (part of `FormatPisOptions`) left-pads the value with zeros to the full 11 digits before masking (default `false`). `options.obfuscate` (same type, default `false`) hides the first 3 digits and the check digit (`***.45678.90-*`), after `pad`. No authority publishes a masking rule for the PIS, so this applies the one [Lei nº 12.309/2010, art. 87, § 5º](https://www.planalto.gov.br/ccivil_03/_ato2007-2010/2010/lei/l12309.htm) sets for the CPF ("ocultar os três primeiros dígitos e os dois dígitos verificadores"), a number with the same structure. It is read for truthiness, so any truthy value obfuscates.
...
Format CNH. `options.pad` (part of `FormatCnhOptions`) left-pads the value with zeros to the full 11 digits before masking (default `false`). `options.obfuscate` (same type, default `false`) hides the first 3 digits and the 2 check digits (`***503064-**`), after `pad`. No authority publishes a masking rule for the CNH, so this applies the one [Lei nº 12.309/2010, art. 87, § 5º](https://www.planalto.gov.br/ccivil_03/_ato2007-2010/2010/lei/l12309.htm) sets for the CPF ("ocultar os três primeiros dígitos e os dois dígitos verificadores"), a number with the same structure. It is read for truthiness, so any truthy value obfuscates.
...
Format a voter ID number. Uses the 12-digit grouping `0000 0000 00 00` by default; the 13-digit grouping `0000 0000 0 00 00` is used only when the sanitized value has more than 12 digits **and** its federative union code (the 10th and 11th digits) is `01` (São Paulo) or `02` (Minas Gerais), the two states whose voter ids may carry a 9-digit sequential number. `options.obfuscate` (part of `FormatVoterIdOptions`, default `false`) hides the first 3 digits and the 2 check digits (`***4 5678 01 **`), leaving the federative union code visible. No authority publishes a masking rule for the voter id, so this applies the one [Lei nº 12.309/2010, art. 87, § 5º](https://www.planalto.gov.br/ccivil_03/_ato2007-2010/2010/lei/l12309.htm) sets for the CPF ("ocultar os três primeiros dígitos e os dois dígitos verificadores"), a number with the same structure. It is read for truthiness, so any truthy value obfuscates.
...
Hides most of an e-mail address with `*`, for the places where an address is shown to someone who should only recognize it (LGPD, art. 6º III, necessidade). It follows the way the [gov.br account](https://acesso.gov.br/faq/_perguntasdafaq/formarrecuperarconta.html) shows the registered address, `li***********@gm*******`: the first 2 characters of the local part and the first 2 of the domain stay, whatever they are, a dot included, the `@` stays, and every other character, the remaining dots included, becomes one `*`, so the length is preserved. A first label of a single character therefore keeps the dot that follows it visible. The gov.br sample does not cover a local part of 1 or 2 characters, which that rule would show whole, so here such a local part always loses its last character. The value is judged by `isValidEmail` as it comes, with no trimming, and keeps its letter case. Returns an empty string when the value is not a valid e-mail address.
