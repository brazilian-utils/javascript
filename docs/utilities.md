...
A string is only read as a code when it is written as bare digits with optional surrounding whitespace (the field is numeric with 3 digits, [Nota Técnica 2025.002-RTC](https://www.nfe.fazenda.gov.br/portal/listaConteudo.aspx?tipoConteudo=04BIflQt1aY=), field UB13, and has no mask), and a number only when it is a non-negative safe integer. A value narrower than 3 digits is left padded with zeros, as a string or as a number, since the codes start with zeros a numeric field drops: `0`, `'0'` and `'000'` are all the code `000`.
...
Look a CST-IBS/CBS up and get its code and the description the official CST table gives it. Same table and input rules as `isValidCstIbsCbs`.
...
Check if a cClassTrib (Código de Classificação Tributária do IBS e da CBS) exists in the official table, the code the field `cClassTrib` carries next to the CST-IBS/CBS. The table is the cClassTrib sheet of the same workbook the [Portal Nacional da NF-e publishes](https://www.nfe.fazenda.gov.br/portal/listaConteudo.aspx?tipoConteudo=/NJarYc9nus=) (161 classifications in force in the version published on 23/06/2026). Only the classifications in force count: the [Informe Técnico 2025.002](https://www.nfe.fazenda.gov.br/portal/listaConteudo.aspx?tipoConteudo=hXzemuyNHW4=) excludes a classification by closing its validity (`dFimVig`), as v.1.60 did with `220001`, `220002` and `220003`, and those are rejected.

Every classification belongs to exactly one CST-IBS/CBS, the first 3 digits of its code (the Informe Técnico says so: "os três primeiros dígitos são idênticos ao CST-IBS/CBS"), and a document that carries a cClassTrib with another CST is rejected (rejection 1024, "Classificação Tributária do IBS e da CBS incompatível com o CST informado"). Pass the CST the document carries as `options.cst` (part of `IsValidClassTribOptions`) to check the pair too; a `cst` that is given and is not the CST of the classification, whatever it is, makes the result `false`. Omit it to check the cClassTrib alone.

A string is only read as a code when it is written as bare digits with optional surrounding whitespace (the field is numeric with 6 digits, [Nota Técnica 2025.002-RTC](https://www.nfe.fazenda.gov.br/portal/listaConteudo.aspx?tipoConteudo=04BIflQt1aY=), field UB14, and has no mask), and a number only when it is a non-negative safe integer. A value narrower than 6 digits is left padded with zeros: `1`, `'1'` and `'000001'` are all the code `000001`. `options.cst` is read the same way, padded to 3 digits.
...
Look a cClassTrib up and get its code, the CST-IBS/CBS it belongs to (`cst`, the first 3 digits), the short name the official table gives for display (`name`, the column "Nome cClassTrib") and the situation it refers to (`description`, the column "Descrição cClassTrib"). The legal wording the workbook also prints for each row (the article of Lei Complementar nº 214/2025 and of both regulations) is not shipped. Same table and input rules as `isValidClassTrib`.
