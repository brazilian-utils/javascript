...
- `src/_internals/` holds the helpers shared between utilities (check-digit arithmetic, `mod10`/
  ...
  (banks, municipalities, CNAE, CBO, CFOP, NCM, CID-10, legal natures), generated by the `scripts/`
  from their official sources and refreshed by the `Update datasets` workflow through
  a pull request, never edited by hand. Every generator runs in the same job, so one of them
  failing opens no pull request at all: when `scripts/cid10.ts` reports a SHA-256 other than the
  reviewed `CID10_ZIP_SHA256` (DATASUS serves that archive over plain HTTP, so the digest is what
  makes the download safe), review the new archive, put its digest in the generator and
  regenerate, otherwise the other datasets stay frozen.
