...
import { yearFromDays } from "./std/date.ts";
...
export function isBusinessDay(
	...
	const year: number = yearFromDays(value);
	if (year < 1900 || year > 2099) {
		return false;
	}
	const weekday: number = ((((value + 3) % 7) + 7) % 7) + 1;
	...
	for (const holiday of getHolidays(year)) {
		...
		if ((holiday.date < value ? -1 : holiday.date > value ? 1 : 0) === 0) {
			return false;
		}
	}
	return true;
