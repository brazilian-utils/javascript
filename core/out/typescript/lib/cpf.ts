 ...
 */
export function cpfCheckDigit(cpf: string): number {
	let sum: number = 0;
	for (let index = 0; index < 9; index++) {
		sum = sum + (cpf.charCodeAt(index) - 48) * (10 - index);
	}
 ...
 * The check digit of a CPF base, under the Receita Federal rule (weights 10..2 and 11..2).
 */
export function cpfCheckDigit1(cpf: string): number {
	let sum: number = 0;
	for (let index = 0; index < 10; index++) {
		sum = sum + (cpf.charCodeAt(index) - 48) * (11 - index);
	}
	const remainder: number = sum % 11;
	return remainder < 2 ? 0 : 11 - remainder;
}
