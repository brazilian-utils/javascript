...
export type CnpjInfo = {
	...
	root: string;
	/** The 4 character branch of the establishment, positions 9 to 12, called "número de ordem" by the Receita Federal. */
	branch: string;
...
const INITIAL_HEADQUARTERS_BRANCH = "0001";
 ...
 * nº 2.229/2024, lays the 14 positions out as 8 (root, raiz, the entity) + 4 (branch, número de
 * ordem, the establishment) + 2 (check digits, always numeric). In the alphanumeric format,
 * assigned to new registrations from July 2026, the root and the branch take the digits `0` to
 ...
 *
 * The branch `0001` marks the headquarters (matriz) only at registration: the Receita Federal
 * Q&A (question 25) states that a filial can become the headquarters while keeping its
 * número de ordem, hence `isInitialHeadquarters` instead of a definitive headquarters flag.
 *
 ...
 * nº 2.119/2022: positions 1 to 8 root, 9 to 12 número de ordem (branch), 13 and 14 check digits.
...
export const getCnpjInfo = (value: string, options?: GetCnpjInfoOptions): CnpjInfo | null => {
	...
	const branch = cnpj.slice(ROOT_END, BRANCH_END);
