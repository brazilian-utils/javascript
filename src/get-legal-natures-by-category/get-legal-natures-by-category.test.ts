...
import { getLegalNaturesByCategory } from "./get-legal-natures-by-category";
...
const codesOf = (category: string | number): string[] =>
	getLegalNaturesByCategory(category).map((legalNature) => legalNature.code);
...
describe("getLegalNaturesByCategory", () => {
	...
	test("should list the legacy codes of the category too, in code order", () => {
	...
	test("should list the 33 codes of Entidades Empresariais, legacy ones in place", () => {
	...
	test("should list the 25 codes of Entidades sem Fins Lucrativos", () => {
	...
	test("should partition the whole table across the five categories", () => {
	...
	describe("properties", () => {
		...
		test("should return the same entry getLegalNature returns for every code it lists", () => {
			fc.assert(
				fc.property(categoryCodes, (category) => {
					for (const legalNature of getLegalNaturesByCategory(category)) {
		...
		test("should read a number category exactly like its string form", () => {
			fc.assert(
				fc.property(categoryCodes, (category) => {
					expect(getLegalNaturesByCategory(Number(category))).toEqual(
						getLegalNaturesByCategory(category),
					);
				}),
			);
		});
