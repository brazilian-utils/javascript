...
export type CliOutput = {
	...
	text: string;
	/** `1` when the value is `false`, `null` or `undefined`, `0` otherwise. */
	exitCode: 0 | 1;
};
 ...
 * `JSON.stringify` would write. The exit code is `1` for the negative answers of the library:
 * `false` from a validator and `null` from a lookup. The values JSON cannot hold (a bigint, a
 * symbol, a function) are printed as their text, so rendering never throws.
 *
