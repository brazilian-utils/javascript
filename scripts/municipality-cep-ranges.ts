...
const withNumericSeparators = (value: number): string =>
...
const main = async (): Promise<void> => {
	...
	const body = allRanges
		.map(
			(range) =>
				`\t{ code: "${range.code}", start: ${withNumericSeparators(range.start)}, end: ${withNumericSeparators(range.end)} },`,
		)
		.join("\n");
