...
/**
 * Renders the two modules the CEST table is shipped as.
 * @param {Record<string, string>} table - The goods descriptions, keyed by the 7 digit code.
 * @param {Record<string, string>} segments - The segment names, keyed by the 2 digit code.
 * @param {string} amendedBy - The line CONFAZ heads the consolidated text with.
 * @returns {Record<string, string>} The content of each generated file, by its path.
 */
export const renderCest = (
 ...
 */
export const CEST_CODES = ${serializeCodes(codes)};
 ...
 * Convênio ICMS 142/18, the consolidated text and its annexes.
 */
export const CEST_DESCRIPTIONS: readonly string[] = ${JSON.stringify(descriptions)};
...
const main = async (): Promise<void> => {
	...
	await writeGeneratedFiles(renderCest(table, segments, amendedBy));
};
