...
/**
 * Renders the two modules the CFOP table is shipped as.
 * @param {Record<string, string>} table - The operation descriptions, keyed by the 4 digits.
 * @returns {Record<string, string>} The content of each generated file, by its path.
 */
export const renderCfop = (table: Record<string, string>): Record<string, string> => {
 ...
 */
export const CFOP_CODES = ${serializeCodes(codes)};
 ...
 * Anexo II of Convênio SINIEF s/nº 1970, the CFOP table in force.
 */
export const CFOP_DESCRIPTIONS: readonly string[] = ${JSON.stringify(descriptions)};
`,
	};
};
...
const main = async (): Promise<void> => {
	...
	await writeGeneratedFiles(renderCfop(sorted));
};
