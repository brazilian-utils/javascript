...
/**
 * Renders the module the NCM codes are shipped in.
 * @param {readonly string[]} codes - The 8 digit codes in force, unique and in ascending order.
 * @returns {Record<string, string>} The content of the generated file, by its path.
 */
export const renderNcm = (codes: readonly string[]): Record<string, string> => ({
	"./src/is-valid-ncm/constants.ts": `/**
 * NCM (Nomenclatura Comum do Mercosul) 8 digit codes in force on the generation date, back to
 * back in ascending order (one per source line, joined by line continuations). A code is included
 * when the generation date falls within its Siscomex \`Data_Inicio\`/\`Data_Fim\` range (both
 * inclusive) — this also keeps codes that are valid today but carry a scheduled future end date,
 * not only the ones with no end date (\`Data_Fim: "31/12/9999"\`).
 *
 ...
 */
export const NCM_CODES = ${serializeCodes(codes)};
...
const main = async (): Promise<void> => {
	...
	await writeGeneratedFiles(renderNcm(uniqueSortedCodes));
};
