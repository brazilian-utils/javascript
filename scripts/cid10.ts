...
import { runAsEntryPoint, splitRecord, writeGeneratedFiles } from "./lookup-table.ts";
...
/**
 * Renders the two modules the CID-10 table is shipped as.
 * @param {Record<string, string>} data - The descriptions, keyed by the code without the dot.
 * @returns {Record<string, string>} The content of each generated file, by its path.
 */
export const renderCid10 = (data: Record<string, string>): Record<string, string> => {
	const { codes, descriptions } = splitRecord(data);
	const subcategories = groupSubcategories(codes);
 ...
 */
export const CID10_DESCRIPTIONS: readonly string[] = ${JSON.stringify(descriptions)};
`,
		"./src/_internals/constants/cid10.ts": `/**
 * Every CID-10 category, in ascending order, with the fourth characters of its subcategories:
 * \`A00: "019"\` stands for \`A00.0\`, \`A00.1\` and \`A00.9\`, and an empty string for a category
 * that is not subdivided. It holds the codes of \`CID10_DESCRIPTIONS\` without the descriptions,
 * in the same order, so that checking a code does not cost the whole table.
 *
 * Generated by \`node ./scripts/cid10.ts\`. Do not edit by hand.
 *
 * @see Official: http://www2.datasus.gov.br/cid10/V2008/downloads/CID10CSV.zip
...
const main = async (): Promise<void> => {
	...
	await writeGeneratedFiles(renderCid10(data));
};
